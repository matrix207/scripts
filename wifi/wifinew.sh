...
usage() {
    ...
    echo "      $PROG -c -e 1 -n wifi-wep -p 12345"
...
adaptercheck () {
	...
	$IFCONFIG $WIFI_INTERFACE 1> /dev/null 2> /dev/null
	if [  $? != 0  ] ; then
    ...
    while getopts ":cdghpsv" opt; do
		case $opt in
			c)
				...
				connet_wireless_network $*
				wifiip
				echo "uwifi -s 4" > /etc/init.d/startwifi
				echo "uwifi $*" >> /etc/init.d/startwifi
				;;
			d)
				checkdeps
				if [ $? -eq 1 ]; then
